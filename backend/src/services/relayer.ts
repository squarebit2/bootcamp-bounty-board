...
import { bountyCreatedEvent, bountyEscrowAbi, escrowFactoryAbi, rewardTokenAbi } from "../contracts";
...
const ensureApproval = async (amount: bigint) => {
  const allowance = await client.readContract({
    ...
    args: [wallet().account.address, CONTRACTS.escrowFactory],
  });
  if (allowance >= amount) return;
  const hash = await wallet().writeContract({
    address: CONTRACTS.rewardToken, abi: rewardTokenAbi, functionName: "approve",
    args: [CONTRACTS.escrowFactory, maxUint256], gasPrice: await gasPrice(),
  });
...
export const createBounty = async (reward: string, rulesURI: string, deadlineJam: number) => {
  ...
  const hash = await wallet().writeContract({
    address: CONTRACTS.escrowFactory, abi: escrowFactoryAbi, functionName: "createBounty",
